...
contains
    ...
    function vmec2000_run_case(this, input_file, output_dir, timeout) result(success)
        ...
        logical :: success
        character(len=:), allocatable :: local_input, prepared_input, cmd, python_script, python_cmd
        ...
        write(unit, '(A)') "    # Run VMEC: restart + readin + timestep + output + cleanup"
        write(unit, '(A)') "    # ictrl[0] bit flags: 1=restart, 2=readin, 4=timestep, 8=output, 16=cleanup"
        write(unit, '(A)') "    ictrl[0] = 1 + 2 + 4 + 8 + 16"
        write(unit, '(A)') "    vmec.runvmec(ictrl, '" // get_basename(local_input) // "', verbose, fcomm, reset_file)"
        write(unit, '(A)') "    # The standalone driver makes a final output call when the last"
        write(unit, '(A)') "    # grid reaches NITER before FTOL.  The Python API leaves that"
        write(unit, '(A)') "    # call to its caller; request the same auditable partial WOUT."
        write(unit, '(A)') "    if ictrl[1] == 2:  # more_iter_flag"
        write(unit, '(A)') "        ictrl[0] = 8 + 16  # output + cleanup"
        write(unit, '(A)') "        ictrl[1] = 11  # successful_term_flag: force complete WOUT"
        write(unit, '(A)') "        ictrl[2] = 0"
        write(unit, '(A)') "        vmec.runvmec(ictrl, '" // get_basename(local_input) // "', verbose, fcomm, reset_file)"
        ...
        cmd = "cd " // trim(output_dir) // " && /usr/bin/time -p timeout --kill-after=5s " // int_to_str(timeout_val) // &
