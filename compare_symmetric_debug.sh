...
# Script to run educational_VMEC, jVMEC, and VMEC++ on the same symmetric input
# with debug output to compare line-by-line for lasym=false cases
...
echo -e "${BLUE}=== VMEC Symmetric Debug Comparison ===${NC}"
echo "This script runs all three VMEC implementations on the same symmetric input"
...
extract_debug_output() {
    ...
    if [ -f "${TEMP_DIR}/educational_vmec/educational_vmec_output.log" ]; then
        ...
        grep -E "DEBUG:|ERROR:|First few|NaN detected|totzsps|symmetric transform" "${TEMP_DIR}/educational_vmec/educational_vmec_output.log" > "${TEMP_DIR}/educational_vmec_debug.txt" || true
    ...
    if [ -f "${TEMP_DIR}/jvmec/jvmec_output.log" ]; then
        ...
        grep -E "DEBUG:|ERROR:|First few|NaN detected|symmetric|BSQ" "${TEMP_DIR}/jvmec/jvmec_output.log" > "${TEMP_DIR}/jvmec_debug.txt" || true
    ...
    if [ -f "${TEMP_DIR}/vmecpp/vmecpp_output.log" ]; then
        ...
        grep -E "DEBUG:|ERROR:|First few|NaN detected|symmetric" "${TEMP_DIR}/vmecpp/vmecpp_output.log" > "${TEMP_DIR}/vmecpp_debug.txt" || true
...
echo -e "\n${GREEN}Symmetric comparison complete!${NC}"
echo "This establishes baseline agreement between implementations for symmetric cases"
echo "before focusing on asymmetric implementation matching."